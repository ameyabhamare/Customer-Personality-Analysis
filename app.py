...
user_id_dropdown = st.sidebar.selectbox("Select User ID", options = analysis_utils.populate_dropdowns())
...
if selected_dropdown == 'Heart Rate':
    ...
    # process data
    heartrate_proc = heartrate_analysis.process_heartrate_data(df_heartrate_unproc, df_sleep_data_unproc, user_id_dropdown)
...
if selected_dropdown == 'Activity & Weight':
    ...
    # process data
    sleep_proc = sleep_analysis.process_sleep_analysis_data(df_sleep_data_unproc, user_id_dropdown)
    daily_steps_proc = steps_analysis.process_daily_steps_data(df_daily_steps_unproc, user_id_dropdown)
    daily_steps_sleep_proc = steps_analysis.process_daily_sleep_steps_data(df_daily_steps_unproc, df_sleep_data_unproc, user_id_dropdown)
    daily_calories_proc = calories_analysis.process_daily_calories_data(df_daily_calories_unproc, user_id_dropdown)
...
if selected_dropdown == 'Caloric Model':
    ...
    # process data
    df_daily_activity_proc = activity_analysis.process_daily_activity_data(df_daily_activity_unproc, user_id_dropdown)
    slider_val = st.slider(selected_c_dropdown, round(min(df_daily_activity_proc[selected_c_dropdown])), round(max(df_daily_activity_proc[selected_c_dropdown])), 1)

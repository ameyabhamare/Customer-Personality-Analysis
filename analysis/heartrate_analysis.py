...
def process_heartrate_data(df_heartrate, df_daily_sleep, user_id = '2026352035'):
    ...
    :return: merged dataset containing heart rate data on a seconds level and sleep information on a daily level
    """
    ...
    heartrate_daily = df_heartrate.resample('1D', on = 'date_time', origin = '2016-04-12 00:00:00').Value.mean().reset_index()
    ...
    # merge datasets
    daily_values = heartrate_daily.merge(df_daily_sleep, how = 'left', on = ['date'])

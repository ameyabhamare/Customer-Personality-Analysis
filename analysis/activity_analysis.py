"""
   Functions for processing and analyzing daily activity data.
"""
import pandas as pd
...
def process_daily_activity_data(df_daily_activity, user_id='1503960366'):
    ...
    Returns:
    - df_daily_activity (pd.DataFrame): Filtered DataFrame with additional columns for year, month,
        and day of week
    '''
    df_daily_activity = analysis_utils.filter_df_user_id(
        df_daily_activity, user_id)

    df_daily_activity['ActivityDate'] = pd.to_datetime(
        df_daily_activity['ActivityDate'])
...
def daily_activity_calories_linreg_model(df, x):
    ...
    '''
    i = df.loc[:, x].values.reshape(-1,
                                    1)  # values converts it into a numpy array
    # -1: calc dimensn of rows, but have 1 column
    j = df.loc[:, 'Calories'].values.reshape(-1, 1)

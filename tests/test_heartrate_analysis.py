...
class TestProcessHeartrateData(unittest.TestCase):
    """
    test suite for heartrate data procesisng
    """
    def setUp(self):
    ...
    def test_process_heartrate_data_filter_user_id(self, mock_filter_df_user_id):
        ...
        mock_filter_df_user_id.side_effect = [
            self.df_heartrate, self.df_daily_sleep]

        # Call the function with the mock data
        analysis.heartrate_analysis.process_heartrate_data(
            self.df_heartrate, self.df_daily_sleep, '1503960366')

        self.assertEqual(len(mock_filter_df_user_id.mock_calls), 2)

    def test_process_heartrate_data(self):
        """
        process data logic is correct
        """
        ...
        output = analysis.heartrate_analysis.process_heartrate_data(
            self.df_heartrate, self.df_daily_sleep, user_id='2026352035')
        self.assertTrue(len(output) == 3)
        self.assertTrue(all(i == j for i, j in zip(
            output['date_time'].to_numpy(), expected_output['date_time'].to_numpy())))
        self.assertTrue(all(i == j for i, j in zip(
            output['Value'].to_numpy(), expected_output['Value'].to_numpy())))
        self.assertTrue(all(i == j for i, j in zip(
            output['day_of_week'].to_numpy(), expected_output['day_of_week'].to_numpy())))
        self.assertTrue(all(i == j for i, j in zip(output['TotalMinutesAsleep'].to_numpy(
        ), expected_output['TotalMinutesAsleep'].to_numpy())))
        self.assertTrue(all(i == j for i, j in zip(
            output['Sleep Duration'].to_numpy(), expected_output['Sleep Duration'].to_numpy())))
